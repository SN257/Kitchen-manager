import { Controller, Get, Post, Put, Delete, Body, Query, Param, Req, UnauthorizedException } from '@nestjs/common';
...
export class EventsController {
  ...
  @Put(':id')
  async update(@Param('id') id: string, @Body() dto: CreateEventDto, @Req() req: Request & { session: any }) {
    const { userId } = req.session;
    if (!userId) throw new UnauthorizedException('Not logged in');
  ...
  @Delete(':id')
  async remove(@Param('id') id: string, @Req() req: Request & { session: any }) {
    const { userId } = req.session;
    if (!userId) throw new UnauthorizedException('Not logged in');
