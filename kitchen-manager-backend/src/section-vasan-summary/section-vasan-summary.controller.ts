...
export class SectionVasanSummaryController {
  ...
  @Get('latest')
  async latest(@Req() req: Request & { session: CustomSession }, @Query('eventId') eventId?: string) {
  ...
  @Post()
  async save(@Req() req: Request & { session: CustomSession }, @Body() dto: CreateSectionVasanSummaryDto) {
    const { userId } = req.session;
    if (!userId) throw new UnauthorizedException('Not logged in');
