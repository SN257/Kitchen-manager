...
export class BoxRangeService {
    ...
    async findOneByUser(id: number, userId: number): Promise<BoxRange> {
        ...
        if (!boxRange) {
            throw new NotFoundException(`BoxRange with ID ${id} not found or access denied`);
        }
        return boxRange;
    }
    ...
    async update(id: number, boxRange: Partial<BoxRange>, userId: number): Promise<BoxRange> {
